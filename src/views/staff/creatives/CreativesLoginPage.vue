...
function redirectAfterLogin() {
  ...
  router.replace(safe ?? { name: creativesDashboardRoute })
}
...
onMounted(async () => {
  ...
  if (auth.hasCreativeAccess) redirectAfterLogin()
})

watch(
  () => auth.hasCreativeAccess,
  (allowed) => {
...
async function handleLogin() {
  ...
  try {
    try {
      await auth.login(identifier.value, password.value, 'CREATIVE')
    } catch {
    ...
    if (!auth.hasCreativeAccess) {
      auth.clearStaffAuth()
      error.value = 'Access denied - this area requires the CREATIVE or ADMIN role.'
      return
    }
...
<template>
  <div class="creatives-login">
    <div class="creatives-login__card">
      <header class="creatives-login__header">
        <div class="creatives-login__logo">
          <img :src="logoUrl" alt="AccSaber" class="creatives-login__logo-img" fetchpriority="high" decoding="async" />
        </div>
        <div>
          <h1 class="creatives-login__title">AccSaber Creatives</h1>
          <p class="creatives-login__subtitle">Creative team access</p>
        </div>
      </header>

      <template v-if="mode === 'providers'">
        <p class="creatives-login__intro">
          Log in with the account you already use on AccSaber. Your creative role is attached
          automatically once an administrator has granted it.
        </p>
        ...
        <p v-if="isPlayerLoggedIn && !auth.hasCreativeAccess" class="creatives-login__note">
          You are signed in, but your account does not have creative access yet.
          Ask an administrator to grant you the CREATIVE role.
        </p>

        <div class="creatives-login__divider"><span>or</span></div>

        <button class="creatives-login__secondary" @click="mode = 'password'">
          Sign in with username &amp; password
        </button>
      </template>

      <form v-else class="creatives-login__form" @submit.prevent="handleLogin">
        ...
        <div v-if="error" class="creatives-login__error" role="alert">
        ...
        <button type="button" class="creatives-login__link" @click="mode = 'providers'">
          Back to OAuth
        </button>
      </form>
    </div>
...
.creatives-login__logo-img {
  width: 48px;
  height: 48px;
  object-fit: contain;
}

.creatives-login__title {
...
.creatives-login__subtitle {
  font-size: var(--text-caption);
  color: var(--text-secondary);
  margin: 2px 0 0;
}

.creatives-login__intro {
  color: var(--text-secondary);
...
.creatives-login__divider::before,
.creatives-login__divider::after {
  content: '';
  flex: 1;
  height: 1px;
...
.creatives-login__secondary:hover {
  border-color: var(--text-tertiary);
  background: var(--bg-elevated);
}

.creatives-login__form {
  display: flex;
  flex-direction: column;
  gap: var(--space-md);
}

.creatives-login__error {
  display: flex;
  align-items: center;
  gap: var(--space-sm);
